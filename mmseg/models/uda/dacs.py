...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        # modified
        self.is_head_expension=cfg['is_head_expension']
        self.unknown_index=cfg['unknown_index']
        ...
        self.is_classmix=cfg['is_classmix']
    ...
    def get_pseudo_label_and_weight(self, logits):
        ...
        if self.is_head_expension:
            pseudo_label = torch.where(pseudo_prob>self.pseudo_thres, pseudo_label, int(self.unknown_index))
        ...
        return pseudo_label, pseudo_weight, ema_softmax, logits ,pseudo_prob
    
    def get_pseudo_label_entropy(self, logits):
        ema_softmax = torch.softmax(logits.detach(), dim=1)
        pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1)
        ...
        pseudo_weight = pseudo_weight * torch.ones(
            pseudo_prob.shape, device=logits.device)
        return pseudo_label, ema_softmax, logits ,pseudo_prob
    ...
    def forward_train(self,
        ...
        if not self.source_only:
            ...
            pseudo_label, pseudo_weight, ema_softmax, logits ,pseudo_prob = self.get_pseudo_label_and_weight(
                ema_logits)
            ...
            # Apply mixing
            mixed_img, mixed_lbl, mixed_gt_semantic_seg = [None] * batch_size, [None] * batch_size, [None]*batch_size
            ...
            if self.is_classmix:
                for i in range(batch_size):
                    ...
                    _, mixed_gt_semantic_seg[i] = strong_transform(
                        strong_parameters,
                        target=torch.stack((gt_semantic_seg[i][0], target_gt_semantic_seg[i][0])))
                del gt_pixel_weight
                mixed_img = torch.cat(mixed_img)
                mixed_lbl = torch.cat(mixed_lbl)
                mixed_gt_semantic_seg=torch.cat(mixed_gt_semantic_seg)
            else:
                for i in range(batch_size):
                    strong_parameters['mix'] = mix_masks[i]
                    mixed_img[i], mixed_lbl[i] = strong_transform(
                    ...
                    _, mixed_seg_weight[i] = strong_transform(
                        strong_parameters,
                        target=torch.stack((pseudo_weight[i], pseudo_weight[i])))
                del gt_pixel_weight
                mixed_img = torch.cat(mixed_img)
                mixed_lbl = torch.cat(mixed_lbl)
        ...
        if self.local_iter % self.debug_img_interval == 0 and \
            ...
            # make image
            source_logits = self.get_ema_model().generate_pseudo_label(
                img, img_metas)
            source_pseudo_label, source_ema_softmax, source_logits ,source_pseudo_prob = self.get_pseudo_label_entropy(
                source_logits)
            
            target_logits = self.get_ema_model().generate_pseudo_label(
                target_img, target_img_metas)
            target_pseudo_label, target_ema_softmax, target_logits ,target_pseudo_prob = self.get_pseudo_label_entropy(
                target_logits)
            
            mixed_logits = self.get_ema_model().generate_pseudo_label(
                mixed_img, img_metas)              
            mixed_pseudo_label, mixed_ema_softmax, mixed_logits ,mixed_pseudo_prob = self.get_pseudo_label_entropy(
                mixed_logits)
            ...
            for j in range(batch_size):
                ...
                subplotimg(
                    axs[0][4],
                    source_pseudo_label[j],
                    'Source Prediction (ST)',
                    cmap='cityscapes')
                subplotimg(
                    axs[0][5],
                    source_pseudo_prob[j],
                    'Conf. scroe (ST)',
                    vmin=0,
                    vmax=1)
                ...
                subplotimg(
                    axs[1][4],
                    target_pseudo_label[j],
                    'Target Prediction (ST)',
                    cmap='cityscapes')
                
                subplotimg(
                    axs[1][5],
                    target_pseudo_prob[j],
                    'Conf. scroe (ST)',
                    vmin=0,
                    vmax=1)
                ...
                if mixed_lbl is not None:
                    subplotimg(
                        axs[2][2], mixed_lbl[j], 'Seg Targ', cmap='cityscapes')
